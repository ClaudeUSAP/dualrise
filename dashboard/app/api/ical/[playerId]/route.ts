...
export async function GET(
  ...
  try {
    ...
    if (!player || player.ical_token !== token) {
    ...
    const { data: events } = await supabase
      ...
      .select(
        'id, title, event_date, event_time, event_type, description, ical_uid, timezone'
      )
    ...
    for (const e of (events ?? []) as CalendarEventRow[]) {
      const uid = `${e.ical_uid ?? e.id}@${HOST}`
    ...
    return new NextResponse(body, {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="usap-dashboard.ics"',
