...
interface BestRecentVsCRDisplayProps {
  athleteId: string;
  showTooltip?: boolean;
  className?: string;
}
...
export function BestRecentVsCRDisplay({ 
  athleteId, 
  showTooltip = true,
  className = "" 
}: BestRecentVsCRDisplayProps) {
  ...
  const fetchScoreData = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('best_recent_period_raw, scoring_avg_vs_cr_last_3, scoring_avg_vs_cr_last_5, scoring_avg_vs_cr_last_7, scoring_avg_vs_cr_last_10, scoring_avg_vs_cr_current_year')
        .eq('id', athleteId)
      ...
      // Use best_recent_period_raw to select the vs CR for the same period as avg score
      if (period) {
        const periodMap: Record<string, string | null | undefined> = {
          'Last 3': data?.scoring_avg_vs_cr_last_3,
          'Last 5': data?.scoring_avg_vs_cr_last_5,
          'Last 7': data?.scoring_avg_vs_cr_last_7,
          'Last 10': data?.scoring_avg_vs_cr_last_10,
          'Current Year': data?.scoring_avg_vs_cr_current_year,
        };
        ...
        if (!bestValue || isNaN(parseFloat(bestValue))) {
          ...
          const { data: computed, error: rpcError } = await supabase.rpc('calculate_scoring_avg_vs_cr_dynamic', {
      ...
      if (!period) {
        const cachedPeriods = [
          { name: 'Last 3', value: data?.scoring_avg_vs_cr_last_3 },
          { name: 'Last 5', value: data?.scoring_avg_vs_cr_last_5 },
          { name: 'Last 7', value: data?.scoring_avg_vs_cr_last_7 },
          { name: 'Last 10', value: data?.scoring_avg_vs_cr_last_10 },
          { name: 'Current Year', value: data?.scoring_avg_vs_cr_current_year }
        ];
        ...
        if (validCachedPeriods.length > 0) {
          const best = validCachedPeriods.reduce((min, p) => {
            return parseFloat(p.value!) < parseFloat(min.value!) ? p : min;
          });
      ...
      setScoreData({
        bestValue,
        bestPeriod: period,
        last3: data?.scoring_avg_vs_cr_last_3 ?? null,
        last5: data?.scoring_avg_vs_cr_last_5 ?? null,
        last7: data?.scoring_avg_vs_cr_last_7 ?? null,
        last10: data?.scoring_avg_vs_cr_last_10 ?? null,
        currentYear: data?.scoring_avg_vs_cr_current_year ?? null,
      });
    } catch (error) {
      console.error('Error fetching best recent vs CR:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const tooltipContent = (
    <div className="space-y-1 text-xs">
      <div className="font-semibold mb-2">Period Breakdown (vs CR):</div>
