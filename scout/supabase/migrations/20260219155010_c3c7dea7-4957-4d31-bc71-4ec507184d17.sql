...
-- Rewrite calculate_scoring_avg_vs_cr_dynamic to use tournament-weighted averaging
...
BEGIN
  ...
  IF filter_type = 'year' THEN
    SELECT 
      ROUND(AVG(
        tr.total_score - t.course_rating::DECIMAL * 
        CASE 
  ...
  ELSIF filter_type = 'last_n' THEN
    SELECT 
      ROUND(AVG(total_score - cr_x_rounds), 2)
    INTO avg_vs_cr
    FROM (
      SELECT 
        tr.total_score,
...
-- Rewrite calculate_scoring_avg_vs_par_dynamic to use tournament-weighted averaging
...
BEGIN
  ...
  IF filter_type = 'year' THEN
    SELECT 
      ROUND(AVG(
        tr.total_score - t.course_par::DECIMAL * 
        CASE 
          WHEN tr.rounds IS NOT NULL AND tr.rounds != '' THEN 
            array_length(string_to_array(tr.rounds, ','), 1)
          WHEN t.field_size IS NOT NULL AND t.field_size::INTEGER <= 30 THEN 3
  ...
  ELSIF filter_type = 'last_n' THEN
    SELECT 
      ROUND(AVG(total_score - par_x_rounds), 2)
    INTO avg_vs_par
    FROM (
      SELECT 
        tr.total_score,
...
-- Rewrite calculate_scoring_avg_dynamic to use tournament-weighted averaging
...
BEGIN
  ...
  ELSIF filter_type = 'last_n' THEN
    SELECT 
      ROUND(AVG(total_score::DECIMAL / num_rounds), 2)
    INTO avg_score
    FROM (
      SELECT 
        tr.total_score,
