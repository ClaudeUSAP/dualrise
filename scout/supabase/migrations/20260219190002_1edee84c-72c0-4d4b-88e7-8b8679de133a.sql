-- Revert to round-weighted scoring: SUM(differential) / SUM(rounds)
...
BEGIN
  ...
  ELSIF filter_type = 'last_n' THEN
    SELECT 
      ROUND(SUM(total_score::DECIMAL) / NULLIF(SUM(num_rounds), 0), 2)
    INTO avg_score
    FROM (
      SELECT 
        tr.total_score,
...
BEGIN
  ...
  IF filter_type = 'year' THEN
    SELECT 
      ROUND(
        SUM(tr.total_score - t.course_rating::DECIMAL * 
          CASE 
            WHEN tr.rounds IS NOT NULL AND tr.rounds != '' THEN 
              array_length(string_to_array(tr.rounds, ','), 1)
  ...
  ELSIF filter_type = 'last_n' THEN
    SELECT 
      ROUND(SUM(total_score - cr_x_rounds) / NULLIF(SUM(num_rounds), 0), 2)
    INTO avg_vs_cr
    FROM (
      SELECT 
        tr.total_score,
...
BEGIN
  ...
  IF filter_type = 'year' THEN
    SELECT 
      ROUND(
        SUM(tr.total_score - t.course_par::DECIMAL * 
          CASE 
            WHEN tr.rounds IS NOT NULL AND tr.rounds != '' THEN 
              array_length(string_to_array(tr.rounds, ','), 1)
            WHEN t.field_size IS NOT NULL AND t.field_size::INTEGER <= 30 THEN 3
  ...
  ELSIF filter_type = 'last_n' THEN
    SELECT 
      ROUND(SUM(total_score - par_x_rounds) / NULLIF(SUM(num_rounds), 0), 2)
    INTO avg_vs_par
    FROM (
      SELECT 
        tr.total_score,
