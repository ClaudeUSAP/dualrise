-- Expose non-PII placement/comparison fields on athletes_safe so coach-facing
-- reads (which use athletes_safe) can build the "Comparable USAP placements"
-- section on an available athlete's profile.
-- Added columns: commit_date, french_adult_ranking_at_commit,
-- french_ranking_1yr_before_college, scoreboard_current_rank.
-- IMPORTANT: athletes_safe is INTENTIONALLY a SECURITY DEFINER view
-- (security_invoker = false). The base table public.athletes is RLS-locked to
-- admin/agent only; coaches read athletes exclusively through this masking view.
-- Definer lets the view expose the rows while the CASE has_role(...) expressions
-- below mask email / phone / date_of_birth. With security_invoker=on the view
-- runs as the coach and returns ZERO rows. The advisor "security_definer_view"
-- warning for this view is accepted. Do NOT set security_invoker=on.
